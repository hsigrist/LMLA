...
\def\talksubtitle{Posiciones relativas: transversal de gravedad}
  ...
  \begin{block}{Definición}
    ...
    \begin{figure}[h]
      ...
      \begin{tikzpicture}[scale=.5]
        ...
        \tkzLabelLine[pos=1.8](A,G){$t_A$}
        \tkzLabelLine[pos=1.8](B,G){$t_B$}
        \tkzLabelLine[pos=1.8](C,G){$t_C$}
     \end{tikzpicture}
  ...
  \begin{minipage}[t]{.6\linewidth}
     \begin{figure}[h]
      ...
      \begin{tikzpicture}
        ...
        \tkzDrawPolygon(A,B,C)
        \tkzCentroid(A,B,C)\tkzGetPoint{G}
        \tkzDrawLines[add = 0 and 3/4,color=blue](A,G)
        \tkzDefMidPoint(B,C) \tkzGetPoint{D}
        \tkzDrawPoint(D)
        \tkzDrawPoint(A)
        \tkzDrawPoint(B)
        \tkzDrawPoint(C)
        \tkzMarkAngle(D,B,A)
        \tkzLabelAngle[pos = .6](D,B,A){$\alpha$}
        \tkzMarkRightAngles(A,D,C)
        \tkzLabelPoint[below right](D){$D$}
        \tkzLabelPoint[below](A){$A$}
        \tkzLabelPoint[right](B){$B$}
        \tkzLabelPoint[above](C){$C$}
        \tkzLabelLine[pos=1.8,above,blue](A,G){$t_A$}
      \end{tikzpicture}
    \end{figure}
  \end{minipage}
  \begin{minipage}[t]{.3\linewidth}
